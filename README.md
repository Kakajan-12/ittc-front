...
Сайт ходит в два разных API — не путать:

| Что | Переменная | Чей |
| --- | --- | --- |
| Регистрация, пакеты, промокоды, оплата, аккаунты участников | `NEXT_PUBLIC_BACKEND_URL` | внешняя платформа |
| Контент сайта, визовые заявки, сервисы участников | `NEXT_PUBLIC_CONTENT_API_URL` | `../ittc-back` |

Клиент регистрации — `shared/api/*` (не трогать). Клиент контента —
`shared/content/*`:

- `queries.ts` — серверные функции, отдают уже локализованные вью-модели
  (`getNews`, `getSpeakers`, `getAgenda`, `getSponsors`, `getPartners`,
  `getFaq`, `getStats`, `getPage`);
- `submit.ts` — браузерные записи: визовая заявка и заявка на сервисы;
- `localize.ts` / `format.ts` — выбор языка из троек `*En/*Ru/*Tk` и даты.

Контент тянется в серверных компонентах с ISR: страница кешируется на
`CONTENT_REVALIDATE_SECONDS` (по умолчанию 300), поэтому правка в админке
появляется на сайте в течение этого окна. Если API недоступен, разделы
отрисуются пустыми, а не уронят страницу.

Тексты в `messages/*.json` — только интерфейсные подписи. Новости, спикеры,
повестка, спонсоры, партнёры, FAQ, счётчики и правовые страницы живут в CMS.
Исключение — блок «О событии»: карточки разделов выставки остались на
переводах, потому что это верстка, а не текстовый блок.

Локальный запуск: поднять `ittc-back` (порт 4000), затем `npm run dev`.
Origin фронта должен быть в `CORS_ORIGINS` бэкенда, иначе браузер заблокирует
отправку визовой формы.
