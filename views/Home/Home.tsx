...
function Home() {
  ...
  return (
    <>
      <section className="relative isolate flex items-center overflow-hidden text-white min-h-[95vh]">
        <Image
          src="/main.jpg"
          alt=""
          fill
        ...
        <div className="container mx-auto px-4 lg:px-10 py-30">
          <div className="max-w-2xl">
            <h1 className="text-4xl font-bold font-roboto leading-tight sm:text-5xl lg:text-6xl">
              {t("title")}
            </h1>

            <p className="mt-1 flex flex-wrap items-center gap-3 text-lg text-white/90 font-roboto">
              <span>{t("date")}</span>
              <span className="text-white">|</span>
              <span>{t("location")}</span>
            </p>

            <div className="mt-8 flex flex-wrap flex-col content-start gap-4 h-66">
              {actions.map(({ key, href }) => (
                <Link
                  key={key}
                  href={href}
                  className="group flex items-center justify-center gap-2 rounded border border-brand-blue w-32 lg:w-36 px-8 py-2.5 text-base transition hover:border-brand-blue hover:bg-brand-blue/20"
                >
                  {t(key)}
